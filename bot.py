...
BOT_NAME = os.getenv('BOT_NAME', 'Lorinplayz')
...
BOT_DEVELOPER = os.getenv('BOT_DEVELOPER', 'Lorinplayz')
...
async def delete_vps(ctx, user: discord.Member, vps_number: int, *, reason: str = "No reason"):
    ...
    # 4️⃣ Success embed
    embed = create_success_embed("🌟 Lorinplayz - VPS Deleted Successfully")
...
async def user_info(ctx, user: discord.Member):
    ...
    embed.set_footer(text="Lorinplayz • User Resource Dashboard")
    embed.timestamp = ctx.message.created_at
...
async def server_stats(ctx):
    ...
    embed.set_footer(text="Lorinplayz • Real-Time Monitoring")
    embed.timestamp = ctx.message.created_at
